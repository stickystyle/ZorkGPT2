...
## Episode 118 → 119 — IMPROVEMENT
**Type:** INCREMENTAL
**Trigger:** ep118 t83 the agent dropped the ivory torch (a +14 treasure it had just taken 28 turns earlier) as chimney "ballast", then spent ~15 turns retrieving it (t84–t100). Same pattern has recurred since ep87 (painting/bar drops at Studio) despite the ep87→88 KB enumerated-treasure-list fix, because each new treasure the agent collects is outside the enumeration. The Score events section of the context already listed "take torch at Torch (+14) turn 55" at t83, but the agent did not consult it before proposing the drop.
**Hypothesis:** The fix belongs in `prompts/agent.md`, not in the KB. The agent needs a reasoning heuristic — parallel to the existing Mandatory pre-movement ritual and pre-combat KB check — that forces it to audit every planned drop target against the Score events section (context) and Score Changes section (KB) before emitting the command. Items that appear as the object of any recorded score delta are "protected" and must be removed from the drop list. This generalizes from ep87→88's enumerated list to a principle, and moves the discipline from KB (which the agent's own synthesis may not keep current) to the agent prompt (read every turn). This is NOT the ep87→88 hypothesis (KB enumeration); it is a different hypothesis about WHERE the rule belongs and HOW it is formulated.
**Change:** Added a new rule (§5 under PRE-ACTION BELIEF CHECK) to `prompts/agent.md`: "Mandatory pre-drop score-event audit". The rule runs before any drop command (`drop X`, `drop X, Y, Z`, `drop all`, `drop all except …`), requires the agent to enumerate each drop candidate in `thinking` with explicit `score-event: YES|NO` tags pulled from the Score events line and KB Score Changes, and HARD-PROHIBITS any score-event item from appearing in the action's drop list. Includes an explicit no-rationalization clause blocking "episode termination", "temporary sacrifice", "just for the climb" overrides. Preserved existing rule 4 (weight/load KB triage) as rule 6 — the two rules are complementary: rule 5 is the mechanical filter, rule 6 is the broader strategic triage. No other files modified.
**Reasoning:** Tests the hypothesis that the agent will apply a mechanical, step-by-step filter ("list each item, tag its score-event status, remove YES items") more reliably than a strategic guideline ("don't drop treasures"). The mechanical form is game-agnostic (any text adventure with a score system has equivalent context) and teaches HOW to think (consult the engine-supplied record before speculating) rather than WHAT to do (no specific items named). Passes prompts/CLAUDE.md two-question test: Q1 yes (applies to any scored text adventure), Q2 no (it is a reasoning procedure, not a strategic directive).
**Target metric:** In ep119, (a) the agent's `thinking` field should contain explicit enumerate-and-tag score-event audit text before every drop command; (b) no item whose short name appears in the Score events section should appear in a drop command's target list — specifically, if the agent reaches Studio with the torch and must climb the chimney, it must NOT drop the torch; it must drop only non-scored items (manual, book, bottle, sack, etc.) or decline to drop and replan.
**Validation:** PASSED (3/3 structural) — t83 (PROBLEM): original `drop torch, book` → new `climb chimney` (refused to drop torch; routed around). t23 (HEALTHY): original `drop leaflet, bloody axe` → new `drop leaflet, bottle` (explicitly audited: "None of these items are recorded in 'Score events this episode' or 'Score Changes' as having produced points yet"; dropped only non-scored items). t99 (HEALTHY): original `drop manual` → new `drop manual` (audit fired: "Manual is not a score-event item"; torch preserved). Required two iterations: first version let the LLM rationalize an override via episode-termination pressure at t99; adding a HARD PROHIBITION preamble and a no-rationalization clause closed that loophole on re-test. All three fixtures replay to ideal behavior.
**Result:** PENDING
