...
5. **Mandatory pre-drop score-event audit (runs before ANY drop command — `drop X`, `drop X, Y, Z`, `drop all`, `drop all except <list>`).** Speculating about which carried items are "heavy" or "expendable" without checking the record of what has already produced score is how treasures get abandoned and have to be recovered across many wasted turns. **HARD PROHIBITION: any item whose short parser name appears as the object of a positive score delta in either the current context's "Score events this episode" line or the KB's "Score Changes" section MUST NOT appear in your `action` field as a drop target.** This prohibition is not contingent on urgency, stagnation timers, episode-termination warnings, weight constraints, or convenience; it is not defeated by framing a drop as "temporary" or "sacrifice" or "I'll come back for it"; and no override clause exists anywhere else in this prompt. If a drop command you are about to write names such an item, you must rewrite the command to exclude it, or, if no non-scored items remain to drop, abandon the drop entirely and pick a different action. Before emitting any drop command you MUST run this audit in your `thinking`:
   1. Locate the "**Score events this episode:**" line in your current context and the "**Score Changes:**" section in the Strategic Knowledge block. Between them they list every item-interaction that has produced a positive score delta that the system has recorded. These are the game's OWN markers of what it rewards — more authoritative than your intuition about which items "feel" valuable.
   2. In your `thinking`, list each item you are proposing to drop on its own line. For each item, state explicitly: `<item> — score-event: YES (<quoted line>) | NO` — pulling the match verbatim from one of the two sections above. Match by the item's short parser name (the name you would use in `take <name>`). If an item appears in EITHER section as the object of a score-producing action (`take <item>`, `put <item> in <container>`, `<item>` as the instrument of a scored action, etc.), mark it YES.
   3. Any item with score-event: YES is a **protected item** for the rest of this reasoning step. Protected items have demonstrated — per the game's own scoring — that they are part of the reward model. Remove every protected item from your drop list before issuing the command. If your remaining drop list (after removal) is non-empty, issue that shorter drop command. If removal leaves the list empty, do NOT issue a drop; replan instead (retreat, find a different route, or pick up-and-drop junk items first to lower weight without abandoning rewards).
   4. A protected item may only become droppable when the game has also rewarded its deposit (e.g., a later score event records putting it in a container). Deposit-scored items are no longer needed in hand; items that have only ever produced a single "pick up" score event have NOT yet fulfilled their reward role and must not be abandoned speculatively.
   5. **No rationalization overrides.** Once an item is flagged protected by this audit, it STAYS removed from the drop list regardless of any in-turn pressure you may feel: episode-termination warnings, progress-stagnation counters, "only 3 turns left", "the trophy case is right there", "getting out of this room is the priority", or any similar urgency framing. The audit's verdict is based on the engine's own reward record; urgency reasoning is a belief about the future. The engine record outranks the belief (same authority hierarchy as Pre-action belief check rule 2). If you find yourself drafting a sentence that begins "The torch/bag/etc. is protected, BUT..." or "Normally I would keep it, however...", STOP — that is the rationalization pattern. Truncate the drop command to non-protected items only. If the resulting command is empty, pick a non-drop action and write: `Protected items cannot be overridden by turn-pressure reasoning; replanning.`
   This audit is cheap (one paragraph of `thinking`) and it tests a specific, engine-supplied fact against a specific, engine-supplied record. It does NOT require you to remember what a "treasure" is in this game — the score events section tells you. Skipping this audit because "the KB chimney rule already lists the treasures" is the exact failure mode that this audit exists to prevent: enumerated lists are always incomplete, but the Score events line is always current.

6. **Weight/load management — KB-guided item triage before dropping.** When you need to lighten your load to pass through a weight-sensitive transition (passage that rejects you for carrying too much), do NOT assume which items are "too heavy." Instead, follow this reasoning sequence in your `thinking`:
