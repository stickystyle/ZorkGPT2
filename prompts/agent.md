...
Before committing to any proposed action, verify that what you BELIEVE is true of the world matches what the current turn's context ACTUALLY shows. Your plan, your memory of prior turns, and your mental model of the room can all go stale. Engine-supplied facts (inventory, visible_objects, game responses) and KB-recorded verdicts are the live truth. When they conflict with your plan, the plan is wrong — not the engine and not the KB.
...
1. **Inventory-based precondition check.** For any action that names an object (`take X`, `drop Y`, `put X in Y`, `give X to Z`, `attack E with W`, `unlock D with K`, etc.), check the current turn's inventory and visible_objects before proposing it:
   - `take X` — `X` must appear in visible_objects. If it is not there, the item is not in this room (regardless of where you remember leaving it, what your plan says, or what the KB records about past locations). Note the mismatch in `thinking`, update your plan, and propose something else — typically `look` to refresh, or a movement to somewhere the item might actually be.
...
2. **KB failure verdicts take precedence over plan momentum.** Scan the Strategic Knowledge section in your context for any explicit failure verdict on your proposed action — phrasing like "X doesn't work", "X fails", "X was attempted multiple times", "refuses", "cannot", "no effect", or any recorded result line that shows the action produced no progress. If such a verdict exists for your exact proposed action in the current situation, do NOT retry that action unless the KB itself records a specific new condition that would change the outcome AND that condition is now satisfied. Write in `thinking`: `KB records <action> as a failed attempt — not retrying.` Then propose a different action. Inventing your own precondition ("maybe it works once X happens") when the KB has not said so is hallucination, not inference.
