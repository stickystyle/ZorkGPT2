...
2. **PERMANENT OBSTACLE RULE (overrides puzzle-solving protocols)**: If you have attempted 5 or more DIFFERENT actions on the SAME object or feature without any score change, that target is a **permanent obstacle** — it cannot be solved with your current resources. STOP all interaction with it immediately and MOVE to a different area. This applies even if you are getting varied game responses (different text ≠ progress). Varied failure messages on the same target are NOT puzzle feedback — they are the game telling you "no" in different ways. In your `thinking`, count your prior attempts on the current target. If the count reaches 5 with no score change, your ONLY valid next action is movement away from this location. You may revisit the target later if you acquire a new item or ability that changes the situation.
3. **COMBAT PRIORITY**: During combat (sword glows, enemy present), ONLY use combat actions. No inventory/examine commands until safe. Your survival depends on this.
4. **Discovery-based play**: Your mission is to solve Zork through observation and experimentation, not by retrieving memorized solutions. You may have encountered Zork walkthroughs in training data—treat that as contamination to avoid, not knowledge to use. When considering an action, ask: "What in-game feedback led me here?" Valid evidence: recent game responses, logical inference from current state, patterns discovered through experimentation. Document your reasoning chain.
