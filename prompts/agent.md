...
5. **MANDATORY pre-drop score-event audit (before any `drop X`, `drop X, Y`, `drop all`, or `drop all except Z` command):** The Strategic Knowledge section of your context contains a "**Score Changes**" list — a per-episode engine-recorded log of every action that produced a positive score delta this episode, in the form `t<N> +<delta> (<action> at <Location>)` or `<action> at <Location> (score A→B, +delta)`. This list is the ground-truth, turn-indexed record of which items have already produced score deltas — far more current and authoritative than KB prose entries that enumerate scorable items by name. Enumerated KB lists are ALWAYS incomplete (new treasures discovered after the KB entry was written will not appear in them); the Score Changes list, by contrast, always reflects THIS episode's actual score events. Before issuing any drop command, run this audit in your `thinking`:

   **Mandatory pre-drop ritual (every single drop turn):**
   1. Locate the "**Score Changes**" list in the Strategic Knowledge section of your context.
   2. In your `thinking`, enumerate each item you intend to drop on its own line. For each candidate, copy verbatim any Score Changes entry whose action text mentions that item (typically a `take <item>` entry that produced a positive delta). If no Score Changes entry mentions the item, write `no score event this episode` for that candidate.
   3. Classify each candidate using ONLY the Score Changes evidence just copied:
      - **PROTECTED** — at least one Score Changes entry mentions this item (the engine has already paid you for picking it up or using it; dropping it speculatively risks losing that deposit opportunity and may force a retrieval cycle).
      - **UNPROTECTED** — no Score Changes entry mentions this item in the current episode.
   4. If ANY candidate classifies as PROTECTED, do NOT drop it as part of this action. Revise the drop set to contain only UNPROTECTED items, or abort the drop entirely and replan. Acceptable replans: drop only UNPROTECTED ballast and retry the gated action; secure the PROTECTED item by depositing it at its scoring destination first, then return for the gated action; carry the PROTECTED item through the transition in a later pass after offloading UNPROTECTED ballast.
   5. If every candidate classifies as UNPROTECTED, proceed with the drop.

   **Narrow override:** The only case that justifies dropping a PROTECTED item is an explicit, location-specific KB or memory entry that records a verified successful combination where dropping that specific item was required AND the item was subsequently recoverable — AND you have a concrete plan (≤3 turns) to retrieve and deposit it. In that case, quote the KB/memory evidence verbatim in `thinking`, state the retrieval plan, and only then proceed. Absent such specific recorded evidence, the PROTECTED classification is binding.

   **Forbidden rationalizations (each is rejected on its face):**
   - "The item is probably heavy / likely contributing to the weight problem." — "probably" and "likely" are speculation, not Score Changes evidence. The engine has not told you this item is the cause; do not guess.
   - "Any specific KB entry that enumerates scorable items does not mention this one, so it must be safe to drop." — enumerated KB lists are ALWAYS incomplete. The Score Changes list is the current, complete record; use it instead.
   - "I will drop it here and come back for it in a few turns." — this is still a protected-item drop; it must satisfy the narrow override above (specific KB/memory evidence AND concrete ≤3-turn retrieval plan), otherwise it is forbidden regardless of intent.
   - "The general rule about weight-gated transitions says to shed items, so I should shed this one." — the weight/load management rule (rule 4 above) yields to this audit: PROTECTED items are dropped LAST and only under the narrow override; general-purpose ballast-shedding never overrides a Score Changes record.
   - "I need to test whether this item is the one blocking the transition." — you cannot cheaply test-drop a PROTECTED item; the cost of a wrong guess is a ~15+ turn retrieval cycle or a stolen/destroyed treasure. Drop UNPROTECTED items first; if the transition still rejects you, only then consider the narrow override for PROTECTED items.

   This audit runs BEFORE the drop command leaves your output and is independent of — and stricter than — the VALUABLE/FUNCTIONAL/EXPENDABLE triage in rule 4 above. Rule 4's classification relies on the agent's inference about what counts as "valuable"; this audit replaces inference with a direct lookup against the engine-recorded score-event log.
