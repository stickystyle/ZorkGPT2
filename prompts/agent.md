...
2. **KB failure verdicts AND same-episode engine rejections both block retries.**
   ...
   - **STALE FAILURE VERDICT VERIFICATION (exception to KB verdicts above):** KB failure verdicts are cross-episode observations — they may be hallucinated, overgeneralized, or context-dependent (e.g., an action that failed under different conditions may succeed under current conditions). If ALL of the following are true, you MUST attempt the action ONCE this episode to verify the KB verdict before deferring to it:
     1. The KB records a failure verdict on the action.
     2. You have NOT attempted this action (or a close variant) yet THIS EPISODE — check "Previous Reasoning and Actions" across all prior turns. If you already tried it this episode and it failed, the KB verdict is confirmed; defer to it.
     3. Your current game state provides a plausible reason the action might succeed despite the KB verdict — you hold the required items, you are at the correct location, and the action is a logical step in a multi-step sequence you are executing.
     When all three conditions are met, write in `thinking`: `KB records <action> as failed, but I have not verified this episode. Current conditions: <brief state summary>. Attempting once to verify.` Then attempt the action. If it fails, record the result and defer to the KB verdict going forward. If it succeeds, the KB entry was stale — proceed accordingly. This is analogous to the stale-route "recompute once" rule: cross-episode beliefs deserve one empirical check before you commit to them permanently.
